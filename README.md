...
- ⚡ **智能片段转码**：渐进式加载，视频秒开，音频异步转码
- 📋 **待压制队列**：预览页面可将多个任务添加到队列，批量开始压制
...
对于需要转码的视频（如 EAC3 音频），采用按需片段转码策略：

| 操作 | 行为 |
|------|------|
| 首次加载 | 只转码前 60秒，约 2-5秒完成 |
| 拖动到 25分钟 | 自动加载 24:55-25:55 片段 |
| 拖动到 40分钟 | 自动加载 39:55-40:55 片段 |
| 同一位置重复访问 | 使用 5分钟缓存，瞬间加载 |

**优势**：
- ✅ 无需等待完整视频转码（50分钟视频只需 2-5秒）
- ✅ 拖动到任意位置快速预览
- ✅ 适合快速对齐字幕和视频
